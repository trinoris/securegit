...
import { generateX25519KeyPair, x25519SharedSecret, type X25519KeyPair } from './identity.js';
...
function buildAad(repoId: string, generation: number, fingerprint: string): Buffer {
 ...
 */
export function wrapForRecipient(opts: WrapForRecipientOptions): WrappedGeneration {
...
export function unwrapForRecipient(opts: UnwrapForRecipientOptions): Buffer {
  let ephPublicKey: Buffer;
  let raw: Buffer;
  try {
  ...
  try {
    ...
    throw new RecipientError(`cannot unwrap: ${(e as Error).message}`);
  }
}
 ...
 */
export function wrapAllGenerations(
  keys: KeySource,
  keyIds: string[],
  recipientPublicKey: Buffer,
  repoId: string,
): Record<string, WrappedGeneration> {
  const out: Record<string, WrappedGeneration> = {};
 ...
 */
export function unlockFromRecipientFile(
  file: RecipientFile,
  identityKeyPair: X25519KeyPair,
  repoId: string,
): KeySource {
  const held = new Map<string, Buffer>();
  let highest: { generation: number; keyId: string } | null = null;
...
export function recipientsDir(repoDir: string): string {
  return join(repoDir, '.securegit', 'recipients');
}

export function recipientPath(repoDir: string, fingerprint: string): string {
 ...
 */
export async function writeRecipientFile(path: string, file: RecipientFile): Promise<void> {
...
export async function readRecipientFile(path: string): Promise<RecipientFile> {
  const raw = await readFile(path, 'utf8');
  return JSON.parse(raw) as RecipientFile;
}
...
export function removedRecipientsLogPath(repoDir: string): string {
  return join(repoDir, '.securegit', 'removed-recipients.json');
}

export async function readRemovedRecipientsLog(path: string): Promise<RemovedRecipientLogEntry[]> {
  try {
    return JSON.parse(await readFile(path, 'utf8')) as RemovedRecipientLogEntry[];
  } catch {
    return [];
  }
}
...
export async function appendRemovedRecipientLogEntry(
  path: string,
  entry: RemovedRecipientLogEntry,
): Promise<RemovedRecipientLogEntry[]> {
