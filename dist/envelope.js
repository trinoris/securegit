 ...
 */
function padContent(content, padTo) {
...
export function seal(plaintext, opts) {
    ...
    const { ciphertext, authTag } = aeadEncrypt(dek, tag.subarray(0, NONCE_LEN), content, buildAad(header, boundPath));
...
export function unseal(envelope, opts) {
    ...
    const content = aeadDecrypt(dek, header.tag.subarray(0, NONCE_LEN), header.ciphertext, header.authTag, buildAad(envelope.subarray(0, header.headerLength), boundPath));
