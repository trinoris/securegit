...
export function encodePacket(payload) {
    if (payload.length > MAX_PACKET_PAYLOAD) {
        throw new PktLineError(`pkt-line payload of ${payload.length} bytes exceeds the ${MAX_PACKET_PAYLOAD}-byte maximum`);
    }
...
export function encodePacketList(items) {
...
export class PktLineReader {
    ...
    readList() {
        ...
        if (flushIndex === -1)
            return undefined;
        const packets = this.queue.splice(0, flushIndex);
...
//# sourceMappingURL=pktline.js.map