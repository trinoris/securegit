...
import { createCipheriv, createDecipheriv, createHash, createHmac, hkdfSync, timingSafeEqual, } from 'node:crypto';
...
export function deriveTagKey(rmk) {
 ...
 */
export function contentTag(tagKey, plaintext, path) {
...
export function deriveFileKey(rmk, tag, path) {
    requireBytes(rmk, KEY_LEN, 'master key');
    requireBytes(tag, CONTENT_TAG_LEN, 'content tag');
    const p = normalizePath(path);
    const info = p === null ? INFO_DEK : Buffer.concat([INFO_DEK, SEP, Buffer.from(p, 'utf8')]);
 ...
 */
export function keyFingerprint(rmk) {
    requireBytes(rmk, KEY_LEN, 'master key');
...
export function aeadDecrypt(key, nonce, ciphertext, authTag, aad) {
    requireBytes(key, KEY_LEN, 'key');
    requireBytes(nonce, NONCE_LEN, 'nonce');
