 ...
 */
async function headSignerFingerprint(repoDir) {
    try {
        const { stdout } = await execFile('git', ['-c', 'gpg.ssh.allowedSignersFile=/dev/null', 'log', '-1', '--format=%GF', 'HEAD'], { cwd: repoDir });
 ...
 */
async function commitSignedByRecipientCheck(repoDir) {
