 ...
 */
export function exportRecovery(opts) {
 ...
 * result.
 */
export function importRecovery(file, code, expectedRepoId) {
    if (file.repoId !== expectedRepoId) {
        throw new RecoveryError(`securegit: this recovery file belongs to repository ${file.repoId}\n` +
    ...
    for (const [genStr, entry] of Object.entries(file.generations)) {
        ...
        if (!Number.isInteger(generation))
            continue;
        try {
            const rmk = aeadDecrypt(wrapKey, Buffer.from(entry.nonce, 'hex'), Buffer.from(entry.ciphertext, 'hex'), Buffer.from(entry.authTag, 'hex'), aad);
...
export function recoveryFilePath(repoDir, filename) {
 ...
 */
export async function writeRecoveryFile(path, file) {
...
export async function appendRecoveryLogEntry(path, entry) {
