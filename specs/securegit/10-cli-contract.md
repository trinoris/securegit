...
| `identity init` records an existing `git config user.signingkey` without prompting | `src/cli.test.ts` | — | ✅ |
| `identity init` prints guidance, generates nothing, when no signing key is found and `--generate-signing-key` wasn't passed | `src/cli.test.ts` | — | ✅ |
| `identity init --generate-signing-key` is a no-op (not an overwrite) when a signing key is already recorded | `src/cli.test.ts` | — | ✅ |
...
| `key add-recipient --signing-key` records the field; omitting it leaves the recipient exactly as before this feature | `src/cli.test.ts` | — | ✅ |
