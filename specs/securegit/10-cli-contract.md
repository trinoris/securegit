...
| Command | Effect |
|---|---|
| `securegit identity init [--label <label>] [--generate-signing-key]` | Generate an X25519 keypair, wrap the private half via the same `KeyProvider` port a repository uses, write `~/.securegit/identity.json`. Refuses if one already exists. **🔲 Spec only, not built:** also checks `git config --get user.signingkey` and records it (no prompt — reading an existing public key is not a new secret) if present; prints guidance rather than acting if absent. `--generate-signing-key` generates a fresh Ed25519 SSH-format signing keypair only when none is already recorded (a no-op, not a silent overwrite, otherwise) — see [08-multi-recipient.md](08-multi-recipient.md)'s "Commit signing". |
...
| `securegit key add-recipient <pubkey> [--label <label>] [--signing-key <ssh-public-key>]` | Wraps every generation the caller currently holds for a recipient's public key ([08](08-multi-recipient.md)), writes `.securegit/recipients/<fingerprint>.json`. Refuses a malformed public key or a locked repository. `addedBy` is the caller's own identity fingerprint if `identity init` has been run locally, else blank — not an error, since the caller may only have direct keyring access. `--signing-key` (**🔲 spec only, not built**) records this recipient's signing public key alongside the existing one — optional, so a recipient added before this existed keeps working exactly as today, just without a `signingKey` field for `verify`'s authenticity check to match against. |
...
| `identity init` records an existing `git config user.signingkey` without prompting | `src/cli.test.ts` | — | 🔲 spec only |
| `identity init` prints guidance, generates nothing, when no signing key is found and `--generate-signing-key` wasn't passed | `src/cli.test.ts` | — | 🔲 spec only |
| `identity init --generate-signing-key` is a no-op (not an overwrite) when a signing key is already recorded | `src/cli.test.ts` | — | 🔲 spec only |
...
| `key add-recipient --signing-key` records the field; omitting it leaves the recipient exactly as before this feature | `src/cli.test.ts` | — | 🔲 spec only |
