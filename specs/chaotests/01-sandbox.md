...
| actor-3 | Operator | Never edits `secrets/` files. Periodically runs `key rotate --confirm-recipients <n>`, `verify --access`, `securegit status`, `key add-recipient` for actor-1/actor-2's identities — the maintenance role, run concurrently with actors 1/2's ordinary traffic over real wall-clock time and a real network, not one `Promise.all` batch. Also re-runs `protect` against the full protected-pattern list every round, unconditionally followed by `reencrypt` — a no-op pair once nothing is stale, and the T1 recovery (plus a general stale-generation cleanup — see below) whenever something is (`reconcileProtection()` in `chaos/actors/driver.mjs`; see [16](../securegit/16-adversarial-integrity.md)'s T1 section for why this lives in the ops role rather than the product, and for why `reencrypt` has to run every round unconditionally rather than only when `.gitattributes` itself changed). |
...
- [03](03-orchestrator.md) — extends this single-branch, direct-push
  workflow with two more (a shared working branch, a PR-gated `master`)
  and specifies the merge-request-reviewing `orchestrator` role only the
  latter two admit
